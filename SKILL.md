...
2. **Anchor every payoff to a real stake.** Fill `rationale` with one line per
 ...
 4. **Structure = horizon and noise, as ranges.**
   ...
   - `reputation` (optional): **memory as social standing** — each side keeps a private opinion (good/bad) of the other, updated by every observed move (Ohtsuki–Iwasa *Leading Eight*, default `L3` stern-judging). A bad mark forces punishment (D); optional `gossip` rate lets opinions converge to a consensus; optional `quantitative` + `theta` switches to a numeric ledger (Hilbe score: C=+1, D=−1, cooperate if score ≥ θ). Use when the story says "they remember what you did and tell others" — sanctions regimes, trade blocs, industry cartels with shared blacklists. Shape: `"reputation": {"norm": "L3", "gossip": [0.10, 0.25], "quantitative": true, "theta": 1}`. **Read it in Stage 3 as:** cooperation survives noise only if a misread can be repaired through gossip; with quantitative θ>0 one slip is punished harder than image norms.
   - `temporal` (optional): **contact rhythm** — how many rounds happen per "snapshot" of the relationship (activity-driven networks, Li et al. 2021). `g≈1` = sporadic, bursty contact (cooperation suffers; defects go unpunished); `g≥5` = dense repeated contact (cooperation strengthens). Use when the story is about meeting frequency/density: "they trade daily" vs "formal summit once a year". Shape: `"temporal": {"g": [1, 2.5]}` or `"g": [4, 6]}`. **Read it in Stage 3 as:** if rhythm is bursty, noise effectively bites harder — shorten w advice to "meet more often or write it down".
