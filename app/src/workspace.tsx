    ...
    <main className="river-pane">
      ...
      {centerTab === "context" ? <div className="context-pane">
        ...
        <div className="river-below">
          {selectedAnalysis && !selectedAnalysis.adapter && <div className="river-detail"><div className="river-tools"><button type="button" onClick={() => void replaySelectedWorld()} disabled={replayMutation.isPending || !selectedAnalysis.artifactUrl}>{replayMutation.isPending ? "Replaying…" : selectedAnalysis.artifactUrl ? `Replay world #${(riverSelection?.worldIds[0] ?? 0) + 1}` : "New run required"}</button><button type="button" onClick={() => void relabelAnalysis()} disabled={relabelPending || !selectedAnalysis.artifactUrl || analysisActive} title={agentAvailable ? "Regenerate branch labels with Pi" : agentStatusText}>{relabelPending ? "Relabeling…" : "Relabel"}</button></div>{replayError && <div className="error" role="alert">{replayError}</div>}{worldReplay && <WorldReplayCard value={worldReplay} />}{taskId && selectedAnalysis?.id && selectedAnalysis.artifactUrl && outcomeFacts.length > 0 && <RunPosteriorCard taskId={taskId} analysisId={selectedAnalysis.id} trials={selectedAnalysis.trials} outcomeCount={outcomeFacts.length} />}</div>}
...
function RunCard({ analysis, currentRevision, selected, onClick, onDelete }: { analysis: TaskState["analyses"][number]; currentRevision: number; selected: boolean; onClick: () => void; onDelete: () => void }) {
  ...
  const metricName = analysis.primaryMetric ?? "cooperation";
  const metricValue = analysis.metrics?.[metricName]?.mean ?? analysis.cooperation.mean;
  const metricDisplay = analysis.metrics?.[metricName] ? metricValue.toFixed(2) : `${Math.round(metricValue * 100)}%`;
  const bar = Math.max(0, Math.min(100, metricValue * (analysis.metrics?.[metricName] ? 100 : 100)));
  return <div className="run-row"><button className={`run-item ${stale ? "stale" : ""}`} aria-current={selected} onClick={onClick}><div className="run-top"><b>{analysis.trials} {word(analysis.trials, "world", "worlds", "worlds")}</b><span>{compactDate(analysis.completedAt)}</span></div><div className="run-summary"><div className="run-metric"><strong>{metricDisplay}</strong><span>{metricName}</span></div><span className="run-revision">{stale ? "outdated" : `r${analysis.revision}`}</span></div><div className="run-bar"><i style={{ width: `${bar}%` }} /></div><div className="run-footer">{leader ? <span className="run-leader">{leader[0]} · {Math.round(leader[1])}% wins</span> : <span className="run-leader">{analysis.adapter ? "Generic simulation" : "No clear leader"}</span>}<span>seed {analysis.seed}</span></div></button><button className="icon quiet row-delete" onClick={onDelete} aria-label={`Delete run with ${analysis.trials} worlds`}><Icon name="trash" /></button></div>;
}
