...
function StrategicModelReview({ model }: { model: ScenarioModel }) {
  ...
  return <section className="decision-model strategic-model" aria-label="Strategic interaction model review">
    <header><div><span className="eyebrow">Strategic interaction</span><h2>{model.situation}</h2></div><div className="decision-objective"><small>Repeated game</small><b>{(model.game ?? "prisoners_dilemma").replaceAll("_", " ")}</b><span>continuation {range(model.structure.w)}</span></div></header>
...
function DecisionModelReview({ model, onPin }: { model: DecisionModel; onPin?: (p: PinnedContext) => void }) {
  ...
  return <section className="decision-model" aria-label="Decision model review">
    <header><div><span className="eyebrow">Decision model</span><h2>{model.question}</h2></div><div className="decision-objective"><small>{model.objective.direction === "minimize" ? "Minimize" : "Maximize"}</small><b>{model.objective.label}</b>{model.objective.target !== undefined && <span>target {model.objective.target}{model.objective.unit ? ` ${model.objective.unit}` : ""}</span>}</div></header>
